...
bot.action(/^ver_(.+)$/, async (ctx) => {
  ...
  if (isPremium(user)) {
    // Show account picker before connecting
    setState(ctx.from.id, { ...state, step: 'pick_account', version });
    const accounts = db.getAccounts(ctx.from.id);
    return showAccountPicker(ctx, state.host, state.port, version, accounts);
  }

  clearState(ctx.from.id);
  await doConnect(ctx, state.host, state.port, version, null);
});

async function showAccountPicker(ctx, host, port, version, accounts) {
  const rows = accounts.map(a =>
    [Markup.button.callback(`👤 ${a.nick}`, `pick_acc_${a.id}`)]
  );
  rows.push([Markup.button.callback('➕ Новый ник', 'acc_new_nick')]);
  if (accounts.length === 0) {
    rows.push([Markup.button.callback('🎮 Стандартный ник (whminebot-N)', 'acc_default')]);
  } else {
    rows.push([Markup.button.callback('🎮 Стандартный ник', 'acc_default')]);
  }
  rows.push([Markup.button.callback('❌ Отмена', 'main')]);
  await ctx.editMessageText(
    `👤 *Выбери аккаунт для подключения*\n\n` +
    `🌐 Сервер: ` + code(host + ':' + port) + `\n` +
    `📦 Версия: ` + esc(version) + `\n\n` +
    (accounts.length === 0 ? '_Нет сохранённых аккаунтов_' : `Сохранено: ${accounts.length} аккаунтов`),
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard(rows) }
  );
}

async function doConnect(ctx, host, port, version, chosenNick) {
  ...
  let mcUsername;
  if (chosenNick) {
    // Explicit nick chosen from account manager — check for conflicts only
    const existing = mc.getActiveBotsForUser(userId);
    const takenNicks = existing.map(inst => inst.bot?.username?.toLowerCase());
  ...
  } else if (isPremium(user) && user.custom_bot_nick) {
    const existing = mc.getActiveBotsForUser(userId);
    const takenNicks = existing.map(inst => inst.bot?.username?.toLowerCase());
    const baseNick = user.custom_bot_nick;
    if (takenNicks.includes(baseNick.toLowerCase())) {
      let suffix = 2;
      while (takenNicks.includes((baseNick + suffix).toLowerCase())) suffix++;
      mcUsername = baseNick + suffix;
    } else {
      mcUsername = baseNick;
    }
  } else {
...
bot.on('text', async (ctx) => {
  ...
  // Account manager: enter nick for THIS connection (not saved)
  if (state.step === 'acc_enter_nick') {
    const nick = text.replace(/[^a-zA-Z0-9_]/g, '').slice(0, 16);
    if (nick.length < 3) return ctx.reply('❌ Ник слишком короткий (мин. 3 символа).');
    const { host, port, version } = state;
    clearState(userId);
    // Ask if user wants to save this nick
    return ctx.reply(
      `👤 Ник: ` + code(nick) + `\n\nСохранить в менеджере аккаунтов?`,
      {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
          [Markup.button.callback('💾 Сохранить и подключить', `acc_save_connect_${nick}_${host}_${port}_${version}`)],
          [Markup.button.callback('🎮 Просто подключить', `acc_nosave_connect_${nick}_${host}_${port}_${version}`)],
          [Markup.button.callback('❌ Отмена', 'main')],
        ])
      }
    );
  ...
  // Account manager: add new saved nick
  if (state.step === 'acc_add_nick') {
    const nick = text.replace(/[^a-zA-Z0-9_]/g, '').slice(0, 16);
    if (nick.length < 3) return ctx.reply('❌ Ник слишком короткий (мин. 3 символа).');
    clearState(userId);
    try {
      db.createAccount(userId, nick);
      return ctx.reply(`✅ Аккаунт ` + code(nick) + ` сохранён!`, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('👤 Менеджер аккаунтов', 'acc_manager')]]) });
    } catch (e) {
      return ctx.reply(`❌ Ник ` + code(nick) + ` уже есть в списке.`, { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('👤 Назад', 'acc_manager')]]) });
    }
  }
...
// Pick saved account → connect with that nick
bot.action(/^pick_acc_(\d+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  ...
  const state = getState(ctx.from.id);
  clearState(ctx.from.id);
  await doConnect(ctx, state.host, state.port, state.version, account.nick);
});

// Use default whminebot-N nick
bot.action('acc_default', async (ctx) => {
  await ctx.answerCbQuery();
  const state = getState(ctx.from.id);
  clearState(ctx.from.id);
  await doConnect(ctx, state.host, state.port, state.version, null);
});

// Enter new nick (and optionally save)
bot.action('acc_new_nick', async (ctx) => {
  await ctx.answerCbQuery();
  setState(ctx.from.id, { ...getState(ctx.from.id), step: 'acc_enter_nick' });
  await ctx.editMessageText(
    '✏️ *Введи ник для этого подключения:*\n\nОн будет использован в игре. Хочешь — сохраним в менеджере аккаунтов.',
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([[Markup.button.callback('❌ Отмена', 'main')]]) }
  );
});

// Account manager — full menu (from main menu or settings)
...
async function showAccountManager(ctx) {
  ...
  text += accounts.length
    ? accounts.map((a, i) => `${i+1}. \`${a.nick}\``).join('\n')
    : '_Нет сохранённых аккаунтов_';
  text += `\n\nСохранено: ${accounts.length}/${maxAccounts}`;

  const rows = [];
  if (accounts.length < maxAccounts) {
    rows.push([Markup.button.callback('➕ Добавить аккаунт', 'acc_add')]);
  }
  // Delete buttons
  for (const a of accounts) {
    rows.push([Markup.button.callback(`🗑 Удалить ${a.nick}`, `acc_del_${a.id}`)]);
  }
...
bot.action('acc_add', async (ctx) => {
  await ctx.answerCbQuery();
  setState(ctx.from.id, { step: 'acc_add_nick' });
  await ctx.editMessageText(
    '➕ *Добавить аккаунт*\n\nВведи Minecraft-ник (только a-z, 0-9, _):',
...
// Save & connect / Just connect from account picker text input
bot.action(/^acc_save_connect_([^_]+)_(.+)_(\d+)_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const [, nick, host, port, version] = ctx.match;
  db.createAccount(ctx.from.id, nick);
  await doConnect(ctx, host, parseInt(port), version, nick);
});

bot.action(/^acc_nosave_connect_([^_]+)_(.+)_(\d+)_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  const [, nick, host, port, version] = ctx.match;
  await doConnect(ctx, host, parseInt(port), version, nick);
});
